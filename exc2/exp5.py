...
struc = [(means[0], d, multivariate_normal.pdf(d, means[0]))
         for d in data[0:10]]
...
def Maximization(X, alpha_list):
    ...
    new_cov = np.array([[np.matrix(x-new_mean).T*np.matrix((x-new_mean)) for alpha, new_mean in zip(alphas, new_means)]
                        for alphas, x in zip(alpha_list, X)])
